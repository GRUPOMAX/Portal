// src/modules/config/banner-empresarial/EditPage.tsx
...
type Rec = BannerEmpresarial & { Id?: number | string; id?: number | string };
...
export default function EditPage() {
  ...
  useEffect(() => {
    ...
    if (!isNew && id) {
      (async () => {
        try {
          const rec = await readRecord<BannerEmpresarial>(TABLE_ID, id);
  ...
  async function save() {
    try {
      ...
      if (isNew) {
        await createRecord<BannerEmpresarial>(TABLE_ID, form);
      } else {
        const Id = (form.Id ?? form.id) as number | string;
        await updateRecord<BannerEmpresarial>(TABLE_ID, { ...form, Id });
      }
      navigate('..');
    } catch (e: any) {
      setErr(e.message ?? String(e));
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => navigate(-1)}
            className="h-10 px-3 rounded-xl border border-white/10 hover:bg-white/5 inline-flex items-center gap-2"
          >
          ...
          <button
            onClick={save}
            disabled={busy}
            className="h-10 px-4 rounded-xl bg-emerald-500/80 hover:bg-emerald-500 inline-flex items-center gap-2"
          >
